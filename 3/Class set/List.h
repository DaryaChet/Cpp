...
template<typename T> bool List<T>::isEmpty() { return root == nullptr || (root == root->nextNode && root == root->previousNode); };//	проверка на пустоту,
template<typename T> size_t List<T>::getSize() { return size; };//	количество элементов в списке,
